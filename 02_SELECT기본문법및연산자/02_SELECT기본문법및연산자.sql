...
SELECT /* SELECT 절, 뒤에는 컬럼 */
       *
  FROM EMPLOYEE; /* FROM 절, 뒤에는 테이블 */
 /* 전체는SELECT 구문 */
  
  -- 특정 컬럼 조회
  -- EMPLOYEE 테이블 사번, 이름 조회
SELECT 
       EMP_ID
     , EMP_NAME
  FROM EMPLOYEE;
...
SELECT
       *
  FROM EMPLOYEE
WHERE JOB_CODE = 'J1'; /* 들어가는 값은 대소문자 구분 */
...
-- 단, 숫자, 특수문자 등이 틀어가는 별칭을 이용하는 경우 ""로 별칭을 감싸야 한다.
-- AS 생략 가능함
SELECT
       EMP_NAME AS 이름
     , SALARY * 12 AS "1년급여"
 ...
 */
 ...
 -- J2 직급의 급여 200 만원 이상 받는 직원이거나 
 -- J7 직급인 직언의 이름, 급여, 직급코드 조회
 SELECT
       EMP_NAME 이름
     , SALARY 급여
     , JOB_CODE 직급코드

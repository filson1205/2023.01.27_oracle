...
SELECT
 ...
 -- 성별별 급여 평균(정수처리), 급여 합계, 인원수를 조호한 뒤 
 -- 인원수로 내림차순 정렬하세요
...
SELECT 
 ...
 -- 인자로 전달받은 컬럼 집함의 산출물이 0을 반환하고, 아니면 1을 반환하는 함수 
 SELECT
        DEPT_CODE
      , JOB_CODE
      , SUM(SALARY)
      , GROUPING(DEPT_CODE) "부서별그룹묶인상태"
      , GROUPING(JOB_CODE) "직급별그룹묵인상태"
  FROM EMPLOYEE
 GROUP BY CUBE(DEPT_CODE, JOB_CODE)
 ORDER BY DEPT_CODE;
...
-- INTERSECT : 여러 개의 SELECT한 결과에서 공통 부분만 결과로 호출 
--             수학에서의 교집합과 비슷하다.
SELECT /* 호출 함수의 정보 */
       EMP_ID
     , EMP_NAME

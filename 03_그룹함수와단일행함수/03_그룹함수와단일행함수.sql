-- 그룹함수와 단일행함수
-- 단일행 함수(SINCLE ROW FUNCTION) : N개의 값을 넣어 N개의 결과를 리턴
-- 그룹 함수(GROUP FUNCTION) : N개의 값을 널어 1개의 결과를 리턴
...
-- 단일행 함수
-- 문자관련 함수
-- : LENGTH, LENGTHB, SUMSTR, UPPER, LOWER, INSTR
SELECT LENGTH(EMAIL)
     , LENGTHB(EMAIL)
  FROM EMPLOYEE;
...
-- TRIM : 주어진 컬럼이나 문자열의 아ㅠ/뒤에 지정한 문자를 제거
...
-- CONCAT : 문자열 주 개를 하나의 문자열로 함친 후 리턴
SELECT CONCAT('가나다라', 'ABCD') FROM DUAL;

SELECT '가나다라' || 'ABCD' FROM DUAL;

-- REFLACE : 컬럼 혹은 문자열을 입력받아 변경하고자 하는 문자열을 바꾼 후 리텬
SELECT REPLACE('서울시 강남구 역삼동', '역삼동', '삼성동') FROM DUAL;

-- 숫자처리 함수 : ABS, MOD, ROUND, FLOOR, TRUNC, CEIL ...
-- ABS(숫자 | 칼럼명) : 절대값 구하는 함수
SELECT ABS(- 10)
     , ABS(10)
  FROM DUAL;
...
SELECT EMP_NAME
     , HIRE_DATE
     , CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) 입사개월수
  FROM EMPLOYEE;
...
-- EXTRACT(YEAR FROM 날짜) : 년도 만 추출
-- EXTRACT(MONTH FROM 날짜) : 월 만 추출
-- EXTRACT(DAY FROM 날짜) : 날짜 만 추출
...
SELECT TO_CHAR(SYSDATE, 'YEAR, Q') || '분기'
  FROM DUAL;
...
SELECT TO_DATE('20101010', 'RRRRMMDD')
  FROM DUAL;

SELECT TO_CHAR(TO_DATE('20101010', 'RRRRMMDD'), 'RRRR, MON')
  FROM DUAL;
...
-- TO_NUMBER(문자, [포맷]) : 문자 데이터를 숫자로 리턴
...
-- 직원 정보에서 보너스 포인트가 NULL인경우 직원은 0.5로
...
-- 그외 직급의 직원은 5%만 인상한다.
-- 직원 테이블에서 직원명, 직급코드, 급여, 인산급여를 조회하세요
SELECT EMP_NAME
     , JOB_CODE
     , SALARY
...
-- 사번, 사원명, 급여를 EMPLOYEE테이블에서 조회하고
-- 급여가 500만원 초과이면 '고급'
-- 급여가 300~500만원이면 '중급'
-- 그 이하는 초급으로 출력되도록 하고 별칭은 '구분'으로 한다.
